...
use crate::pyfunction::{
    PyCallee, PyDecorator, PyFunction, PyFunctionBody, PyParam, PyTsThisParam,
};
...
use crate::pyprop::{
    PyAssignProp, PyBigIntPropName, PyComputedPropName, PyExprOrSpread, PyGetterProp,
    PyIdentPropName, PyKeyValueProp, PyMemberProp, PyMethodProp, PyNumPropName, PyProp, PyPropName,
    PyPropOrSpread, PyPropOrSpreadProp, PySetterProp, PyShorthandProp, PySpreadElement,
    PyStrPropName, PySuperProp,
};
use crate::pystmts::stmt_to_py;
use crate::pystmts::{PyCatchClause, PyForHead, PySwitchCase, PyVarDeclOrExpr};
use crate::pytypeinfo::{
    PyTplElement, PyTsArrayType, PyTsCallSignatureDecl, PyTsConditionalType,
    PyTsConstructSignatureDecl, PyTsConstructorType, PyTsEntityName, PyTsExprWithTypeArgs,
    PyTsFnType, PyTsGetterSignature, PyTsImportCallOptions, PyTsImportType, PyTsIndexSignature,
    PyTsIndexedAccessType, PyTsInferType, PyTsIntersectionType, PyTsLit, PyTsLitBigInt,
    PyTsLitBool, PyTsLitNumber, PyTsLitStr, PyTsLitTpl, PyTsLitType, PyTsMappedType,
    PyTsMethodSignature, PyTsOptionalType, PyTsParenthesizedType, PyTsPropertySignature,
    PyTsQualifiedName, PyTsRestType, PyTsSetterSignature, PyTsThisTypeOrIdent,
    PyTsThisTypeOrIdentIdent, PyTsThisTypeOrIdentThis, PyTsTupleElement, PyTsTupleType,
    PyTsTypeElement, PyTsTypeLit, PyTsTypeOperator, PyTsTypeParam, PyTsTypeParamDecl,
    PyTsTypeParamInstantiation, PyTsTypePredicate, PyTsTypeQuery, PyTsTypeQueryExpr,
    PyTsTypeQueryExprEntityName, PyTsTypeQueryExprImport, PyTsUnionType,
};
...
pub fn conv_private_name(py: Python<'_>, name: PrivateName) -> PyResult<PyPrivateName> {
    return Ok(PyPrivateName {
        span: conv_span(py, name.span)?,
        name: conv_atom(py, name.name)?,
    });
}
...
pub fn conv_tstype(py: Python<'_>, ts_type: TsType) -> PyResult<Py<PyTsType>> {
    let base = PyTsType {};
    Ok(match ts_type {
        TsType::TsKeywordType(e) => Py::new(py, (PyTsKeywordType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsThisType(e) => Py::new(py, (PyTsThisType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsFnOrConstructorType(TsFnOrConstructorType::TsFnType(e)) => {
            Py::new(py, (PyTsFnType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsFnOrConstructorType(TsFnOrConstructorType::TsConstructorType(e)) => {
            Py::new(py, (PyTsConstructorType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsTypeRef(e) => Py::new(py, (PyTsTypeRef::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsTypeQuery(e) => Py::new(py, (PyTsTypeQuery::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsTypeLit(e) => Py::new(py, (PyTsTypeLit::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsArrayType(e) => Py::new(py, (PyTsArrayType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsTupleType(e) => Py::new(py, (PyTsTupleType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsOptionalType(e) => Py::new(py, (PyTsOptionalType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsRestType(e) => Py::new(py, (PyTsRestType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsUnionOrIntersectionType(TsUnionOrIntersectionType::TsUnionType(e)) => {
            Py::new(py, (PyTsUnionType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsUnionOrIntersectionType(TsUnionOrIntersectionType::TsIntersectionType(e)) => {
            Py::new(py, (PyTsIntersectionType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsConditionalType(e) => Py::new(py, (PyTsConditionalType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsInferType(e) => Py::new(py, (PyTsInferType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsParenthesizedType(e) => {
            Py::new(py, (PyTsParenthesizedType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsTypeOperator(e) => Py::new(py, (PyTsTypeOperator::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsIndexedAccessType(e) => {
            Py::new(py, (PyTsIndexedAccessType::build(py, e)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        TsType::TsMappedType(e) => Py::new(py, (PyTsMappedType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsLitType(e) => Py::new(py, (PyTsLitType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsTypePredicate(e) => Py::new(py, (PyTsTypePredicate::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsType::TsImportType(e) => Py::new(py, (PyTsImportType::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_block_stmt(py: Python<'_>, stmt: BlockStmt) -> PyResult<Py<PyBlockStmt>> {
    let base = PyStmt {
        stmt: Stmt::Block(stmt.clone()),
    };
    let sub = PyBlockStmt::build(py, stmt)?;
    Py::new(py, (sub, base))
}

pub fn conv_option_block_stmt(
    py: Python<'_>,
    stmt: Option<BlockStmt>,
...
pub fn conv_pat(py: Python<'_>, pat: Pat) -> PyResult<Py<PyPat>> {
    let base = PyPat {};
    Ok(match pat {
        Pat::Ident(p) => Py::new(py, (PyBindingIdentPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Array(p) => Py::new(py, (PyArrayPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Rest(p) => Py::new(py, (PyRestPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Object(p) => Py::new(py, (PyObjectPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Assign(p) => Py::new(py, (PyAssignPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Invalid(p) => Py::new(py, (PyInvalidPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Pat::Expr(p) => Py::new(py, (PyExprPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_propname(py: Python<'_>, name: PropName) -> PyResult<Py<PyPropName>> {
    let base = PyPropName {};
    Ok(match name {
        PropName::Ident(n) => Py::new(py, (PyIdentPropName::build(py, n)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        PropName::Str(n) => Py::new(py, (PyStrPropName::build(py, n)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        PropName::Num(n) => Py::new(py, (PyNumPropName::build(py, n)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        PropName::Computed(n) => Py::new(py, (PyComputedPropName::build(py, n)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        PropName::BigInt(n) => Py::new(py, (PyBigIntPropName::build(py, n)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_object_pat_prop(py: Python<'_>, prop: ObjectPatProp) -> PyResult<Py<PyObjectPatProp>> {
    ...
    Ok(match prop {
        ObjectPatProp::KeyValue(p) => Py::new(py, (PyKeyValuePatProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ObjectPatProp::Assign(p) => Py::new(py, (PyAssignPatProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ObjectPatProp::Rest(p) => Py::new(py, (PyObjectPatRestProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_object_pat_props(
    py: Python<'_>,
    props: Vec<ObjectPatProp>,
) -> PyResult<Vec<Py<PyObjectPatProp>>> {
    props
        .into_iter()
        .map(|p| conv_object_pat_prop(py, p))
        .collect()
}

pub fn conv_option_catch_clause(
    py: Python<'_>,
...
pub fn conv_expr(py: Python<'_>, expr: Expr) -> PyResult<Py<PyExpr>> {
    ...
    Ok(match expr {
        Expr::This(e) => Py::new(py, (PyThisExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Array(e) => Py::new(py, (PyArrayLitExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Object(e) => Py::new(py, (PyObjectLit::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Fn(e) => Py::new(py, (PyFnExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Unary(e) => Py::new(py, (PyUnaryExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Update(e) => Py::new(py, (PyUpdateExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Bin(e) => Py::new(py, (PyBinExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Assign(e) => Py::new(py, (PyAssignExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Member(e) => Py::new(py, (PyMemberExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::SuperProp(e) => Py::new(py, (PySuperPropExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Cond(e) => Py::new(py, (PyCondExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Call(e) => Py::new(py, (PyCallExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::New(e) => Py::new(py, (PyNewExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Seq(e) => Py::new(py, (PySeqExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Ident(e) => Py::new(py, (PyIdentExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Lit(e) => Py::new(py, (PyLitExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Tpl(e) => Py::new(py, (PyTpl::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TaggedTpl(e) => Py::new(py, (PyTaggedTpl::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Arrow(e) => Py::new(py, (PyArrowExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Class(e) => Py::new(py, (PyClassExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Yield(e) => Py::new(py, (PyYieldExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::MetaProp(e) => Py::new(py, (PyMetaPropExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Await(e) => Py::new(py, (PyAwaitExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Paren(e) => Py::new(py, (PyParenExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::JSXMember(e) => Py::new(py, (PyJSXMemberExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::JSXNamespacedName(e) => Py::new(py, (PyJSXNamespacedName::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::JSXEmpty(e) => Py::new(py, (PyJSXEmptyExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::JSXElement(e) => Py::new(py, (PyJSXElement::build(py, *e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::JSXFragment(e) => Py::new(py, (PyJSXFragment::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsTypeAssertion(e) => Py::new(py, (PyTsTypeAssertion::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsConstAssertion(e) => Py::new(py, (PyTsConstAssertion::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsNonNull(e) => Py::new(py, (PyTsNonNullExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsAs(e) => Py::new(py, (PyTsAsExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsInstantiation(e) => Py::new(py, (PyTsInstantiation::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::TsSatisfies(e) => Py::new(py, (PyTsSatisfiesExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::PrivateName(e) => Py::new(py, (PyPrivateNameExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::OptChain(e) => Py::new(py, (PyOptChainExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Expr::Invalid(e) => Py::new(py, (PyInvalidExpr::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_option_boxed_tstype(
    py: Python<'_>,
    ts_type: Option<Box<TsType>>,
...
pub fn conv_fn_param(py: Python<'_>, param: TsFnParam) -> PyResult<Py<PyPat>> {
    let base = PyPat {};
    Ok(match param {
        TsFnParam::Ident(p) => Py::new(py, (PyBindingIdentPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsFnParam::Array(p) => Py::new(py, (PyArrayPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsFnParam::Rest(p) => Py::new(py, (PyRestPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        TsFnParam::Object(p) => Py::new(py, (PyObjectPat::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_option_boxed_ts_this_param(
    py: Python<'_>,
    node: Option<Box<TsThisParam>>,
) -> PyResult<Option<Py<PyTsThisParam>>> {
    match node {
        None => Ok(None),
...
pub fn conv_option_function_body(
    py: Python<'_>,
...
pub fn conv_prop(py: Python<'_>, prop: Prop) -> PyResult<Py<PyProp>> {
    let base = PyProp {};
    Ok(match prop {
        Prop::Shorthand(i) => Py::new(py, (PyShorthandProp::build(py, i)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Prop::KeyValue(p) => Py::new(py, (PyKeyValueProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Prop::Assign(p) => Py::new(py, (PyAssignProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Prop::Getter(p) => Py::new(py, (PyGetterProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Prop::Setter(p) => Py::new(py, (PySetterProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        Prop::Method(p) => Py::new(py, (PyMethodProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_prop_or_spread(py: Python<'_>, node: PropOrSpread) -> PyResult<Py<PyPropOrSpread>> {
    ...
    Ok(match node {
        PropOrSpread::Spread(s) => Py::new(py, (PySpreadElement::build(py, s)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        PropOrSpread::Prop(p) => Py::new(py, (PyPropOrSpreadProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_prop_or_spreads(
    py: Python<'_>,
    nodes: Vec<PropOrSpread>,
) -> PyResult<Vec<Py<PyPropOrSpread>>> {
    nodes
        .into_iter()
        .map(|n| conv_prop_or_spread(py, n))
        .collect()
}
...
pub fn conv_expr_or_spreads(
    py: Python<'_>,
    elems: Vec<Option<ExprOrSpread>>,
) -> PyResult<Vec<Option<Py<PyExprOrSpread>>>> {
    elems
        .into_iter()
        .map(|opt| opt.map(|eos| conv_expr_or_spread(py, eos)).transpose())
        .collect()
}

pub fn conv_expr_or_spreads_noopt(
    py: Python<'_>,
    elems: Vec<ExprOrSpread>,
) -> PyResult<Vec<Py<PyExprOrSpread>>> {
    elems
        .into_iter()
        .map(|eos| conv_expr_or_spread(py, eos))
        .collect()
}
...
pub fn conv_boxed_type_param_instantiation(
    py: Python<'_>,
...
pub fn conv_key(py: Python<'_>, key: Key) -> PyResult<Py<PyKey>> {
    match key {
        Key::Private(p) => Py::new(
            py,
            PyKey {
                private: Some(conv_private_name(py, p)?),
                public: None,
            },
        ),
        Key::Public(p) => Py::new(
            ...
            PyKey {
                private: None,
                public: Some(conv_propname(py, p)?),
            },
        ),
    }
}
...
pub fn conv_class_member(py: Python<'_>, member: ClassMember) -> PyResult<Py<PyClassMember>> {
    ...
    Ok(match member {
        ClassMember::Constructor(c) => Py::new(py, (PyConstructor::build(py, c)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::Method(m) => Py::new(py, (PyClassMethod::build(py, m)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::PrivateMethod(m) => Py::new(py, (PyPrivateMethod::build(py, m)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::ClassProp(p) => Py::new(py, (PyClassProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::PrivateProp(p) => Py::new(py, (PyPrivateProp::build(py, p)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::TsIndexSignature(s) => {
            Py::new(py, (PyClassIndexSignature::build(py, s)?, base))?
                .into_bound(py)
                .into_super()
                .unbind()
        }
        ClassMember::Empty(e) => Py::new(py, (PyClassEmptyMember::build(py, e)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::StaticBlock(s) => Py::new(py, (PyStaticBlock::build(py, s)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
        ClassMember::AutoAccessor(a) => Py::new(py, (PyAutoAccessor::build(py, a)?, base))?
            .into_bound(py)
            .into_super()
            .unbind(),
    })
...
pub fn conv_class_members(
    py: Python<'_>,
    members: Vec<ClassMember>,
) -> PyResult<Vec<Py<PyClassMember>>> {
    members
        .into_iter()
        .map(|m| conv_class_member(py, m))
        .collect()
}
...
pub fn conv_var_declarators(
    py: Python<'_>,
    nodes: Vec<VarDeclarator>,
) -> PyResult<Vec<Py<PyVarDeclarator>>> {
    nodes
        .into_iter()
        .map(|n| conv_var_declarator(py, n))
        .collect()
}

pub fn conv_ts_interface_body(
    py: Python<'_>,
...
pub fn conv_ts_enum_members(
    py: Python<'_>,
    nodes: Vec<TsEnumMember>,
) -> PyResult<Vec<Py<PyTsEnumMember>>> {
    nodes
        .into_iter()
        .map(|n| conv_ts_enum_member(py, n))
        .collect()
}
...
pub fn conv_ts_module_ref(py: Python<'_>, node: TsModuleRef) -> PyResult<Py<PyTsModuleRef>> {
    match node {
        ...
        TsModuleRef::TsExternalModuleRef(r) => {
            let external = Py::new(
                py,
                PyTsExternalModuleRef {
                    span: conv_span(py, r.span)?,
                    expr: conv_str(py, r.expr)?,
                },
            )?;
            Py::new(
                py,
...
pub fn conv_jsx_member_expr(py: Python<'_>, node: JSXMemberExpr) -> PyResult<Py<PyJSXMemberExpr>> {
    let base = PyExpr {
        expr: Expr::JSXMember(node.clone()),
    };
...
pub fn conv_jsx_element(py: Python<'_>, node: JSXElement) -> PyResult<Py<PyJSXElement>> {
    let base = PyExpr {
        expr: Expr::JSXElement(Box::new(node.clone())),
    };
...
pub fn conv_jsx_fragment(py: Python<'_>, node: JSXFragment) -> PyResult<Py<PyJSXFragment>> {
    let base = PyExpr {
        expr: Expr::JSXFragment(node.clone()),
    };
...
pub fn conv_option_expr_or_spreads_noopt(
    py: Python<'_>,
    elems: Option<Vec<ExprOrSpread>>,
...
pub fn conv_boxed_tpl(py: Python<'_>, tpl: Box<Tpl>) -> PyResult<Py<PyTpl>> {
    let base = PyExpr {
        expr: Expr::Tpl(*tpl.clone()),
    };
    let sub = PyTpl::build(py, *tpl)?;
    Py::new(py, (sub, base))
}

pub fn conv_arrow_function_body(
    py: Python<'_>,
...
pub fn conv_boxed_arrow_function_body(
    py: Python<'_>,
...
pub fn conv_switch_cases(
    py: Python<'_>,
    nodes: Vec<SwitchCase>,
